...
danBtn.addEventListener('click',()=>dan99(Number(danInput.value))) //대입값
...
//style을 같은 DOM요소에 연속해서 두번 이상 적용하고 싶은 경우 CSS속성명을 자바스크립트 키워드 방식의 Camel표기법으로 작성해야 한다.
...
console.log(header.children[0].innerHTML); //안에 들어있는 태그 포함 모든 것 
console.log(header.children[0].innerText); //안에 들어있는 내용 (텍스트)
console.log(header.children[1].innerHTML);
//header에 마우스 올렸을 때 자식h1의 "logo" -> "로고" 글자 변경하기
header.addEventListener('mouseover',()=>header.children[0].children[0].innerText='로고');
console.log('=================childElementCount 자식노드개수');
console.log(header.childElementCount); //2 (장바구니갯수, 상품갯수 등에 사용)
...
cart_num_all.innerText = cart_num; //초기값 0 선언
//2. 사용자가 물건 2개를 담았다.
